...
@jit(nopython=True)
def _gbm_simulation_jit(S0: float, mu: float, sigma: float, T: float, 
                        n_steps: int, n_simulations: int, seed: int) -> np.ndarray:
    ...
    for i in range(n_simulations):
        for t in range(1, n_steps + 1):
            ...
            paths[t, i] = paths[t-1, i] * np.exp((mu - 0.5 * sigma**2) * dt + 
                                                   sigma * np.sqrt(dt) * Z)
