...
from . import normal_distributions as norm
...
Examples:
  ...
  rss-calc glossary --term "p-value"
    ...
    stats_parser.add_argument('--data', required=True, 
                             help='Data values (comma or space separated)')
    stats_parser.add_argument('--stat', choices=['mean', 'median', 'std', 'summary'],
                             help='Specific statistic to calculate')
    stats_parser.add_argument('--all', action='store_true',
                             help='Calculate all descriptive statistics')
    
    # Probability subcommand
    prob_parser = subparsers.add_parser('prob', help='Probability calculations')
    prob_parser.add_argument('--type', required=True,
    ...
    prob_parser.add_argument('--std', type=float, default=1, help='Standard deviation (default: 1)')
    prob_parser.add_argument('--cdf', action='store_true', help='Calculate CDF instead of PDF')
    ...
    test_parser.add_argument('--data', required=True,
                            help='Sample data (comma or space separated)')
    test_parser.add_argument('--type', required=True, choices=['t_test'],
    ...
    # Glossary subcommand
    glossary_parser = subparsers.add_parser('glossary', help='Look up statistical terms')
    glossary_parser.add_argument('--term', required=True,
                                help='Statistical term to look up')
