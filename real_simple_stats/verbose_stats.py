...
def t_test_verbose(
    ...
    if verbose:
        print(f"\nStep 3: Calculate T-Statistic")
        print(f"-" * 70)
        print(f"Formula: t = (x̄ - μ₀) / SE")
        print(f"         t = ({sample_mean:.4f} - {mu_null:.4f}) / {standard_error:.4f}")
    ...
    if verbose:
        print(f"\nStep 7: Decision")
        print(f"-" * 70)
        print(f"Compare p-value to significance level:")
        print(f"  p-value = {p_value:.6f}")
        print(f"  α = {alpha}")
        if reject:
            print(f"\nSince {p_value:.6f} < {alpha}, we REJECT H₀")
            print(f"Conclusion: There is sufficient evidence to reject the null hypothesis")
        else:
            print(f"\nSince {p_value:.6f} ≥ {alpha}, we FAIL TO REJECT H₀")
            print(f"Conclusion: There is insufficient evidence to reject the null hypothesis")
...
def regression_verbose(
    ...
    if verbose:
        ...
        print(f"-" * 70)
        print(f"Formula: b = Σ(x - x̄)(y - ȳ) / Σ(x - x̄)²")
    ...
    if verbose:
        ...
        print(f"P-value: {p_value:.6f}")
        if p_value < 0.05:
            print(f"Since p < 0.05, the relationship is statistically significant")
        else:
            print(f"Since p ≥ 0.05, the relationship is not statistically significant")
